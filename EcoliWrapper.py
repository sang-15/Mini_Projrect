...
count = 0
length = 0
for contig in SeqIO.parse('$HOME/Results/spade_result/contigs.fasta', 'fasta'):
    if len(contig.seq) > 1000:
        count += 1
        length += len(contig.seq)
        long.append(contig)
            
SeqIO.write(long, '$HOME/Results/long.fasta', "fasta")
...
prokka_txt = glob.glob("$HOME/Results/prokka_results/*.txt")
...
fpkm = open('$HOME/Results/transcriptome_data.fpkm', 'w')
writer = csv.writer(fpkm, delimiter = ',')

with open('$HOME/Results/Cufflinks_out/transcripts.gtf') as handle:
